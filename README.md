...
```
# === MÔ TẢ CÁC HÀM ===
+ init_driver(): Khởi tạo trình duyệt Chrome với cấu hình tự động.
+ enhance_image(image): Tăng cường chất lượng ảnh captcha để dễ OCR.
+ read_captcha_from_element(driver, element): Cắt, xử lý và nhận diện captcha bằng Tesseract.
+ go_to_website_and_fill_form(driver, bien_so, loai_xe): Truy cập trang, nhập biển số và loại xe.
+ submit_captcha_and_get_result(driver, captcha_code, bien_so, loai_xe): Gửi form và kiểm tra kết quả trả về.
+ tra_cuu_vi_pham(bien_so, loai_xe): Thực hiện toàn bộ quy trình tra cứu cho từng xe.
+ job(): Chạy tra cứu cho danh sách xe theo lịch.
+ __main__: Lập lịch tra cứu lúc 06:00 và 12:00 mỗi ngày.
